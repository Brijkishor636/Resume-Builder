    ...
    return <div>
        <div className='flex justify-center items-center min-h-screen bg-gradient-to-r from-green-100 via-gray-100 to-green-100'>
            <form onSubmit={handleSubmit} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                <button type="submit" className="mt-2 w-full h-11 cursor-pointer rounded-full text-white bg-green-500 hover:opacity-90 transition-opacity">
                    {state === "login" ? "Login" : "Sign up"}
                </button>
                <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="cursor-pointer text-gray-500 text-sm mt-3 mb-11">{state === "login" ? "Don't have an account?" : "Already have an account?"} <a href="#" className="text-green-500 hover:underline">click here</a></p>
            </form>
        </div>
        <Footer/>
    </div>
